...
def _get_cublas_version():
    ...
    return '10' # PMB: this code cannot parse version 10 of the .so filename
    try:
        print('cublas path=', cublas_path) # PMB
        major, minor = re.search(r'[\D\.]+\.+(\d+)\.(\d+)',
                                 utils.get_soname(cublas_path)).groups()
    except:

        # Create a temporary context to run cublasGetVersion():
        #warnings.warn('creating CUBLAS context to get version number') # PMB removed
        h = cublasCreate()
        version = cublasGetVersion(h)
        print('cublas version=', version) # PMB
        cublasDestroy(h)
        return str(version)
    else:
