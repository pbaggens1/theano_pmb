...
# This is needed as we will hide it later  # PMB added
python_complex = complex  # PMB added
...
def numpy_scalar(data):
    ...
    try:
        python_complex(data)  # works for all numeric scalars # PMB changed np.complex(data) to python_complex(data)
        return data
    except Exception:
...
def isnan(a):
    ...
    if a.dtype in discrete_dtypes:
        return alloc(
            np.asarray(False, dtype="bool_"), *[a.shape[i] for i in range(a.ndim)] # PMB changed bool to bool_
        )
    return isnan_(a)
